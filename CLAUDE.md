...
- ⚠️ **잔여(판단 요청)**: 같은 표를 **세 곳이 각자 파싱**한다 — `service.js` 인라인(라이브) · `naver.js` `scrapeInvestorData`(죽음) · `naverInvestor.js` `parseInvestorRows`(순수·테스트 있음). 죽은 것 제거 + 라이브를 `parseInvestorRows`로 합치는 게 맞지만 표기 정리 범위를 넘어 남겨 뒀다

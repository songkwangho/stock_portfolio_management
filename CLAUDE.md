...
**차트 지표 오버레이 — Phase 2: 지표 레이어 (2026-08-19)**

초보자의 "차트에서 뭘 봐야 할지 모르겠다"를 지표를 **더 넣어서**가 아니라 **그려주고 + 읽는 법을 가르쳐서** 해소한다. 모든 설명은 관찰형·비예측 — Phase 4가 기술·추세·SMA 컷의 방향 예측력 없음을 실증했다.

- [x] **[P2-1]** `GET /stock/:code/history/daily` — 차트용 일봉 **250행**(DB). 창은 `PRICE_CONTEXT_CONSTANTS.RANGE_WINDOW` 재사용(priceContext·`/holdings/volatility`와 같은 표본)
- **판단 — `chart/:timeframe`에 'daily'를 얹지 않고 별도 경로로**: 구 서버는 `tf = timeframe==='monthly' ? 'month' : 'week'`라 **'daily'를 주봉으로 해석**한다. 응답 형태가 같아 클라가 구분할 수 없고, Vercel·Render 배포 순서가 어긋나는 창에서 **주봉을 일봉이라 믿고 그린다**. 새 경로면 404로 명확히 실패하고 40행 폴백으로 내려간다. silent 목록에도 추가(배포 창에 전 사용자 토스트 방지)
- [x] **[P2-2]** `lib/stockDetail/indicatorSeries.ts`(순수) — RSI14·MACD(12,26,SMA9)·볼린저(20,2σ)는 **프로덕션 `indicators.js` 공식을 그대로 롤링화**. 신규: 스토캐스틱(%K14·%D3)·LRC(20일 회귀 ±2σ)·피벗 지지저항·거래량 20일선
- **값 SSOT** — `/indicators`가 최신값 1개만 주므로 클라 재계산이 불가피한데, 공식이 다르면 **차트의 선과 IndicatorPanel의 숫자가 어긋난다**. pg 스텁으로 프로덕션 `calculateIndicators`를 직접 호출해 **4개 시드에서 마지막 값 일치**를 고정(+ 비공회전 대조). 프로덕션의 특이점(RSI 단순평균·MACD signal이 EMA9가 아닌 SMA9·볼린저 모표준편차)을 **고치지 않는다** — 고치면 SSOT가 깨진다
- [x] **[P2-3]** 칩 9종 토글 — 기본 on 4종(캔들·이동평균선·볼린저밴드·거래량), 나머지 opt-in. 칩 본체=그리기, `[?]`=읽는 법 팝업. `lib/stockDetail/chartChips.ts`(순수)가 SSOT
- [x] **[P2-4]** "지금 눈에 띄는 것" 배너 — 거래량 배수·볼린저 위치·5거래일 변동폭 **사실만**. 없으면 "특별히 눈에 띄는 신호가 없어요". 배너 배수와 거래량 평균선이 **같은 창(20)** 을 쓴다
- [x] **[P2-5]** 팝업 확장 — `HelpContent.caveat` 신설(⚠️ 블록). 신규 5키(candle·supportResistance·lrc·stochastic·volume) + 기존 rsi·macd·bollinger·sma 정합. **MACD "추세 전환 신호"·"상승 우위"** / **수급 "큰손들이 미래 가치를 긍정적으로 본다는 신호"**(타인 의도 추정) 제거
- [x] **[P2-6]** 모바일 서브패널 상한 2개 — 초과분은 잘라내고 **안내를 띄운다**(조용히 사라지면 칩이 고장 난 것처럼 보인다). PC는 제한 없음
- **발견 — 볼린저 중심선 == 20일 SMA**(둘 다 최근 20종가 평균 `Math.round`): 같은 선을 색만 달리해 두 번 그리고 있었다 → 이평선 칩이 켜져 있으면 중심선 생략, 팝업에 그 사실 명시. **동일성을 테스트로 못박았다**(근거가 깨지면 선 하나가 조용히 사라진다)
- **금지어** `FORBIDDEN_CHART` = BASE + 과열·기회·오를·내릴·상승/하락/매수/매도 신호·전환 신호. 칩 라벨·배너 전 출력·팝업 본문 전수 스윕. **`caveat`은 제외** — 의도적 부정문이라 부분문자열로 훑으면 경고문 자체를 잡는다(스크리너 캐비엇과 같은 판단). 스윕이 `사라지고`(=disappear)를 `사라`(명령형)로 잡아 문구를 `없어지고`로 교체 — 공용 금지어를 느슨하게 하는 대신
- 검증: node --check · tsc 0 · next build ✓ · **npm test 583**(+113) · **라이브 스크린샷**(로컬 dev + 250행 목 프록시) — 칩 9종·오버레이 전부·서브패널 4개 시간축 정렬·팝업 캐비엇·모바일 상한 안내 **콘솔 에러 0**

**차트 오버레이 — 히스토리 부족 종목 엣지 폴리시 (2026-08-21)**

Phase 2 배포 후 라이브 확인에서 나온 소규모 폴리시 2건. 최근 상장·확대적재 종목(스트라드비젼 475040 **실측 37행**)은 히스토리가 짧아 일부 지표가 **조용히 빠진다**. 전역 "데이터 부족" 칩 라벨은 오버킬(SMA60처럼 부분만 빠지는 정상 케이스까지 붙는다) → targeted 2건만. 칩 비활성화 없음(켜보고 안내를 보는 흐름 유지).

- [x] **[EDGE-1] 범례 정합** — `LineSpec.legend` 신설 + 범례를 **`lines` 배열에서 파생**(`legend && data.length > 0`). 표본 미달 이동평균은 선도 범례도 만들지 않는다. 볼린저·LRC(20봉)도 동일 규칙. **별도 목록을 들지 않는 게 요점** — 목록을 따로 두면 그 순간 드리프트한다(52주=40행·표시-계산 일치 계열)
- ⚠️ **전제 정정 — 이평선 범례는 애초에 없었다**: 지시문은 "범례가 `5·20·60`을 항상 띄운다"였지만 grep·라이브 모두 범례 자체가 없었다(오버레이 라인은 `lastValueVisible:false`). 실제 라이브 증상은 지시문보다 나쁜 쪽 — **세 이평선을 구분할 방법이 없고** 60일선 부재는 표시조차 없었다. 수용 기준 1("범례가 5·20만")을 만족시키려면 범례를 **만들어야** 했다 → 그린 것에서 파생되는 형태로 신설
- [x] **[EDGE-2] 못 그리는 서브패널 안내** — RSI·MACD·스토캐스틱이 표본 미달일 때 **패널을 만들지 않고** `notices`로 넘겨 그 자리에 사실 문구(`panelSampleNotice`: 필요 거래일 + 현재 거래일). 실제 증상은 지시문의 "빈 칸"이 아니라 **패널이 아예 안 생기는 침묵**이었다(`if (r.length) out.push(...)` — else 없음) → 같은 해법
- [x] **[EDGE-3] 보조선 라벨 정합**(지시문 밖, EDGE-1의 같은 결함) — 라벨이 없는 선을 설명하던 3곳: 거래량 `(주황선 = 20일 평균)`(20봉 미만), MACD `(12, 26, 9)`(시그널선은 34봉부터), 스토캐스틱 `(%K 14, %D 3)`(%D는 16봉부터). 주선만 있는 구간용 라벨을 **따로** 두고 빈 시리즈는 넣지 않는다
- **판단 — 문턱 표는 "패널에 뭐라도 그려지는 최소 표본"**: 지시문 표의 스토캐스틱 `~16`은 %D 기준인데 **%K는 14봉부터 그려진다** → 14봉 종목에서 "16거래일이 필요해요"는 거짓 안내다. `PANEL_MIN_BARS.stochastic = 14`로 잡고 %D 지연은 안내가 아니라 **라벨**로 처리(MACD 시그널과 동일 구조). 보조선 문턱은 `STOCH_D_MIN_BARS`·`MACD_SIGNAL_MIN_BARS`로 분리
- **문턱 표 ↔ 계산 함수를 테스트로 묶었다** — 각 지표에서 `n−1`봉이면 빈 배열, `n`봉이면 점이 생기는지 직접 검사. 표의 숫자와 그림의 조건이 갈리면 **안내 문구가 그 순간 거짓말**이 된다. 범례·라벨 문구도 `OVERLAY_LEGEND`/`PANEL_LABEL` 상수로 올려 `FORBIDDEN_CHART` 스윕 안에 넣었다(컴포넌트 인라인이면 스윕 밖으로 샌다)
- 검증: tsc 0 · next build ✓ · **npm test 597**(+14) · **라이브 스크린샷 PC·모바일 콘솔 에러 0** — ① 475040 실데이터 37행: 범례 `5일선·20일선·볼린저밴드`(60 표식 없음), 서브패널 3종 정상 ② 15행(목 절단): 범례 `5일선`만 · 거래량 라벨 축약 · RSI 정상 · 스토캐스틱 `%K`만 + 라벨 고지 · MACD 안내 ③ 12행: 세 패널 전부 안내

**반응형 QA 수정 — 대시보드 도넛·종목상세 헤더 (2026-08-21)**

라이브 반응형 QA(Claude-in-Chrome, iframe 렌더) 결함 2건 + 실측 중 추가로 잡은 3건. 계측은 **Playwright 실뷰포트**(320·360·430·640·763·768·1280)로 재현했고, 보유 데이터는 목 프록시로 주입해 **프로덕션 DB 쓰기 0**.

- [x] **[D1] 자산 배분 카드** — ① `ResponsiveContainer` 제거, `PieChart width={180} height={180}` 고정. 상자가 이미 180×180 고정이라 측정할 게 없고, **부모 레이아웃 확정 전에 0을 재면 차트가 통째로 사라진다**(QA가 iframe에서 본 증상). 고정 px면 그 실패 모드 자체가 없다. ② 좁은 폭 **세로 스택**(도넛 위 / 범례 아래) + 이름에 `title` 부여
- **D1 실측 — 범례 이름 손실은 확실히 재현**: 가로 배치 @360에서 가장 긴 종목명 폭 **0px**(완전 비표시), 나머지도 17~26px(한 글자). "집중 62%"만 남아 **어느 종목인지 알 수 없는 상태**였다. 수정 후 320·360·430 전부 `온전`
- ⚠️ **D1 전제 정정 — 도넛 붕괴는 실뷰포트에서 재현되지 않았다**: 360·763·1280 모두 수정 전에도 180×180 원형(조각 3개)으로 정상 렌더. QA의 "찌그러진 조각/빈 공간"은 **iframe 계측 아티팩트**로 보인다(iframe 레이아웃 확정 전 `ResponsiveContainer`가 0을 잰다). 다만 그게 바로 위 ①이 없애는 실패 모드라 수정은 유효
- **판단 — 스택 경계를 `md`(768) 대신 `sm`(640)**: 지시문은 "md 미만 세로 스택이 가장 안전"이었지만, 640~767에서 도넛 옆이 **크게 비는 시각 퇴행**이 생긴다(그 대역은 수정 전에도 이름이 정상이었다). 640·660·700·763에서 4종목 이름 전부 `온전`을 실측하고 `sm:`으로 잡았다. 수용 기준(360·763 원형 + 이름 표시)은 그대로 충족
- [x] **[D2] 종목상세 헤더** — md 미만 세로 스택 + 종목명 `break-keep`(한글 기본 줄바꿈은 **글자 단위**라 폭이 부족하면 "유유/제약"으로 쪼개진다) + 카테고리·테마 태그·수정 버튼 `whitespace-nowrap` + 수익률은 두 조각을 각각 `nowrap`(통째로 nowrap이면 320px에서 넘치고, 무처리면 "₩4,/469") + 수정 폼 `flex-wrap`. 검증: **320px에서 종목명 1줄**(삼성바이오로직스·8자), 뱃지·버튼·수익률·현재가 전부 1줄
- [x] **[추가-1] 대시보드 히어로 320px 넘침**(지시문 §권장점검 ①) — 계측: 가용폭 288px인데 80px 글리프는 `-25.6%`에 **290px**, `-100.0%`에 **342px**(360px도 `-100.0%` 넘침). `text-[64px] min-[400px]:text-[80px]` — VIS-6의 80px 히어로는 400px 이상에서 그대로. 잔여: **4자리 수익률(+1000% 초과)은 320px에서 여전히 넘친다**(실사용 범위 −100~+999%는 커버)
- [x] **[추가-2] "포트폴리오 수익률 추이" 제목** — 320px에서 "추/이"로 쪼개짐 → `break-keep` + 우측 라벨 `shrink-0`
- **발견 — rect 스캔은 글리프 넘침을 놓친다**: 전 페이지 요소 rect 스캔은 320·360에서 넘침 0이었는데 히어로는 실제로 넘치고 있었다(상자는 뷰포트 안, **글자만** 넘침). `scrollWidth > clientWidth` 스캔을 따로 돌려야 잡힌다 → 11페이지 @320 재스캔, 수정 후 **텍스트 넘침 0**
- 권장 추가 점검 결과: 11페이지 @320·360 **문서 가로 스크롤 0 · 넘침 요소 0** · 종목상세 [요약] 9지표 그리드·[차트·지표] 칩 4행 랩·DisclaimerModal @320(4카드+버튼 전부 접근)·HelpBottomSheet @320(캐비엇+닫기 노출) 정상 · `/journal`·`/watchlist`·`/screener` @320 넘침 없음
- ⚠️ **미해결(의도적 허용)**: 768~1023·xl에서 카드가 좁아(사이드바 272px 점유) 가장 긴 종목명이 `…`로 잘린다. 지시문이 허용한 처리(truncate + `title`)이고 수정 전과 동일 — 완전 숨김은 없다
- 검증: tsc 0 · next build ✓ · **npm test 597** · 라이브 스크린샷 320·360·430·640·763·768·1280 **콘솔 에러 0**(포트폴리오 데이터는 목 프록시 주입 — 프로덕션 무변경)

**반응형 QA 2차 — 다열 그리드·중간 폭 전면 재수행 (2026-08-24)**

1차가 단일 열(360·763)만 봐서 새어나간 결함을 잡는다. **핵심 원인은 하나** — 사이드바가 272px를 먹어 `md`(768)의 본문은 **416px**뿐인데(430px 폰보다 좁다), 앱 곳곳이 `md:`에서 2열로 쪼개고 있었다. `md:` 2열은 구조적으로 성립하지 않는다.

- [x] **[D3] 포트폴리오 홀딩 카드** [P1] — ① 그리드 `md:grid-cols-2` → `xl:grid-cols-2` ② 긴 손익 서술을 `shrink-0` 우측 블록 **밖**으로(카드 전폭 행). 그 블록이 shrink-0인데 안에 최대 40자 문장이 있어 **max-content 375px가 그대로 블록 폭**이 되어 카드를 뚫었다 ③ 종목명 `title` ④ 하단 버튼 `flex-wrap` + `whitespace-nowrap`
- **D3 실측(수정 전)**: 우측 블록이 부모를 **+204px(@768) · +158(@860) · +118(@940) · +76(@1024) · +38(@1100)** 넘침, 1280부터 정상. 육안으로는 보고보다 심각 — **종목명이 0폭으로 사라지고**(좌측이 눌림), 버튼 글자가 세로로 쌓이고, 수익률이 옆 카드 위에 겹쳐 찍혔다
- [x] **[D4] HeaderBar** (신규 발견, 지시문 밖) — **md~1200 전 대역**에서 헤더 내용이 가용폭을 넘어 검색창이 화면 밖으로 잘렸다. 실측 넘침 768 +388 · 860 +296 · 940 +216 · 1024 +132 · 1100 +56. 고정 검색창 420px + 지수 213px + 우측 프로필이 동시에 들어갈 폭이 아니다 → 지수 `md:`→`lg:`, 프로필·구분선 `md:`→`xl:`, 검색창 `md:220 lg:260 xl:420`. 수정 후 전 폭에서 가용폭 이내(282≤416 · 559≤672 · 857≤928)
- [x] **[D2 후속] DetailHeader `md:`→`lg:`** — D2가 만든 회귀. `whitespace-nowrap`을 준 테마 태그가 @768에서 부모를 **+23px** 넘쳤다(2열로 쪼개진 좌측이 ~180px). 같은 근본 원인이라 전환점을 lg로 올려 해소
- [x] **[D5] 공시 제목** (신규) — `break-keep`만 걸려 있어 공백 없는 긴 토큰("[기재정정]유형자산취득결정(종속회사의주요경영사항)")이 @320에서 **+23px** 넘침. `break-words` 병기로 **한 줄에 안 들어가는 토큰만** 넘긴다
- **하네스 — 1차가 못 잡은 3클래스를 직접 계측**: ① **컨테이너 경계 넘침**(자식 right > 부모 content-box right — 뷰포트 넘침 스캔이 놓치는 D3 클래스) ② **글리프 넘침**(`scrollWidth > clientWidth` — rect 스캔이 놓치는 히어로 클래스) ③ **형제 겹침**. Playwright 실뷰포트(iframe은 차트 아티팩트), 보유·관심·거래일지는 목 프록시 주입(**프로덕션 DB 쓰기 0**)
- ⚠️ **하네스 오탐 2건을 스스로 잡았다**: ① SVG 내부(lucide `path` 2개)는 좌표계가 달라 늘 겹침으로 잡힘 → `closest('svg')` 제외 ② **여러 줄로 감긴 inline 요소의 bounding box는 union**이라 같은 문단의 `<span>` 둘이 늘 겹침(`/alerts` 전 폭) → `getClientRects().length > 1` 제외
- ⚠️ **공회전(vacuous clean) 3건을 스스로 잡았다 — 이게 1차 실패의 진짜 형태**: ① `/stocks`가 **HealthGate 스플래시** 상태로 스캔돼 clean ② `/recommendations`가 **페이지 로더** 상태로 clean ③ `/watchlist`·`/journal`이 **빈 상태**라 카드 그리드가 렌더조차 안 됨 → 하네스에 **로더 게이트 + 측정 노드 수 출력**을 넣고, 빈 상태 페이지는 데이터를 주입해 재측정. 스캔 통과만으로 완료 선언하지 않는다
- 검증(전부 경계·글리프·겹침 **0**): 11페이지 × 12폭(320·360·430·640·720·768·860·940·1024·1100·1280·1440) **132/132** · 종목상세 [차트·지표] 12/12 · [기업] 12/12 · 모달 3종(Disclaimer·HelpBottomSheet·PortfolioAddForm) × 8폭 **24/24** · 데이터 채운 그리드(관심종목 12/12 · 거래진단 12/12 · 스크리너 결과 9/9 · 추천 렌즈 버킷 9/9) · 육안 스크린샷 리뷰(@320·768·1024) · tsc 0 · next build ✓ · npm test 597
...
**오픈 게이트 — 카피 중립화 · 크론 워밍 · E2E 스모크 (2026-08-24)**

기술 오픈 블로커 마무리 3건(G1~G3).

- [x] **[G1] InvestorChart 카피 중립화** — "외국인·기관이 함께 매수하면 **긍정적 신호로 보는 경우가 많아요**"를 관찰형으로. Phase 4 세션3이 **수급 축 역방향**을 실증했으니(최강 순매수 구간이 저성과 · train IC −0.014~−0.015 · 버킷 비단조) "플로우=긍정"은 **우리 데이터와 정반대**였다. 문구를 `INVESTOR_SECTION_COPY`(helpTexts.ts) 상수로 올려 스윕 대상에 넣었다 — 컴포넌트 인라인이라 스윕 밖으로 새 있던 것이 원인
- **`FORBIDDEN_FLOW` 신설** = `FORBIDDEN_CHART` + 긍정·부정·**맨 '신호'**·큰손·유리·불리. `'상승 신호'`만 막으면 `"긍정적 신호"`가 그대로 통과한다 — 실제로 그렇게 남아 있었다
- ⚠️ **추가 발견 — G1이 "마지막 잔재"가 아니었다**: `helpTexts`(IndicatorPanel이 쓰는 **라이브** 맵)에 같은 계열이 둘 더 있었다. ① MACD "상승 힘이 강하고… 하락 힘이 강하다는 **신호**예요" ② RSI "**너무 많이** 올랐다/내렸다"(되돌아온다는 함의). 팝업(`HELP_CONTENTS`)에서는 Phase 2에 이미 걷어낸 표현인데 이 맵만 남아 있었다 → 팝업과 **같은 문구**로 정렬 + `tests/stockDetail/helpTexts.test.ts`로 두 표면 모두 스윕(+73 테스트)
- [x] **[G2] 크론 워밍** — `/api/health` 10분 간격, **07:00~24:00 KST**. public 레포라 Actions 분 무료·계정 불필요, `vars.RENDER_API_URL` 오버라이드 + 기본값 폴백. `docs/DEPLOY.md`에 한계·대안 문서화
- ⚠️ **G2는 운영자 1단계 배치가 남았다**: CC 토큰 스코프가 `gist, read:org, repo`뿐이라 GitHub이 `.github/workflows/` 생성을 거부한다(`workflow` 스코프 필요, `gh auth refresh`는 브라우저 인증이라 세션 내 불가). 파일은 `docs/warm-api.workflow.yml`로 커밋했고 **경로만 옮기면 동작**한다(명령 3줄은 파일 머리말·DEPLOY.md에). 내용 검증은 완료 — YAML 구조·cron 5필드·step 셸 `bash -n`
- **판단 — 24시간이 아니라 17시간**: Render 무료는 월 **750 인스턴스-시간**인데 24시간 워밍은 ~720시간으로 여유가 없다. 17h/일(≈510h/월)만 깨워 두고 심야는 자연 sleep
- ⚠️ **워밍 한계 명시**: 인스턴스만 깨운다 — **종목 캐시(TTL 10분)는 안 채운다** → `/stock/:code`는 여전히 미스 시 ~15초(`/recommendations`는 스냅샷이라 무관). GitHub `schedule`은 **지연·누락이 흔해** 15분 sleep 문턱을 항상 이기지 못하고, **60일 무활동 시 자동 비활성화**된다 → 확실히 하려면 외부 크론(UptimeRobot)이 낫다. 근본 해소는 Starter($7) 전환(운영자 결정)
- [x] **[G3] 핵심 플로우 E2E 스모크 14/14 · 콘솔 에러 0** — 온보딩 3갈래(라우팅 검증) · 포트폴리오 CRUD(카드 수·평단 반영 확인) · 종목상세 3탭 · 추천 렌즈 버킷 · 스크리너 프리셋 · 테마 · 알림 · 거래진단 CSV 업로드 · 설정 종목 추가 · 모바일 탭바
- **프로덕션 DB 쓰기 0을 구조로 보장**: 스모크 프록시가 **비-GET을 전부 가로채고** 모르는 변경 경로는 **502로 차단**(업스트림 전송 금지). 실측 27건 가로챔, 미처리 1건은 차단 동작 확인용 probe. 보유 목록은 상태를 들고 있어 CRUD가 화면에 실제 반영되는 것까지 확인
- ⚠️ **스모크 하네스 오탐 4건을 스스로 잡았다**(제품 결함 아님): 온보딩 모달 라벨을 대시보드 빈 상태 CTA 문구로 찾고 있었고("내 주식 관리" vs 실제 "지금 갖고 있는 주식 관리하기"), 면책 버튼은 "확인했습니다"였고, 3번째 갈래는 `/stocks`로 가며, 설정의 `input[type=text].first()`는 **헤더 검색창**을 잡고 있었다(DOM 순서가 먼저다)
- 검증: tsc 0 · next build ✓ · **npm test 670**(+73) · 스모크 14/14 · 콘솔 에러 0

**지표 시각 가이드 — 그림 아이콘 칩 + 삽화 팝업 (2026-08-24)**

초보자의 "차트에서 뭘 봐야 할지 모르겠다"에 **그림으로** 답한다(Phase 2는 그려주고 읽는 법을 글로 가르쳤다). 출발점은 SQMATE의 "모양 카드 + 이게 뭔가요" UX — **UX만 가져오고 예측 프레이밍은 안 가져온다**. SQMATE는 "이 모양이면 오른다"를 가르치지만 Phase 4가 그 방향 예측력 없음을 실증했다. 신규 데이터·계산 0.

- [x] **[GFX-1]** 신규 `lib/stockDetail/indicatorGraphics.tsx` — 그래픽 SSOT. 아이콘 9종(56×34) + 삽화 9종(220×108) + **주석·aria 문자열 상수**. 칩과 팝업이 같은 그림을 쓴다(따로 두면 드리프트 — 이 세션에서 범례·문구가 정확히 그렇게 어긋났다). 색은 `design-tokens.css` hex 그대로라 삽화에서 본 색이 실제 차트에 나온다. 방향색(rise/fall)은 **캔들에만**
- [x] **[GFX-2]** 칩 = **그림 + 이름** 그리드(`3 / sm:5 / xl:9`열). 토글·`[?]`·`aria-pressed` 동작 불변, 두 버튼은 **형제**(중첩 금지 — /themes UX-F 패턴)
- [x] **[GFX-3]** 팝업 상단 삽화 슬롯 → 기존 무엇/어떻게/⚠️ 그대로. 그림 없는 키(PER·PBR·수급 등)는 슬롯 자체가 없다(빈 상자 금지)
- **판단 — on일 때 아이콘 칸은 흰 바탕 유지**: 기존 칩은 on에서 `bg-ink`로 뒤집혔는데, 그걸 카드 전체에 적용하면 **어두운 선으로 그린 아이콘이 안 보인다**. 아이콘 칸은 흰 바탕 + off는 `opacity-40`, **이름 띠만** 반전 — "어두운 배경 = 그려짐"이라는 기존 대비 규칙은 그대로 남는다
- **판단 — 삽화 폭 300px 상한**: viewBox가 시트 폭까지 확대되면 주석 글자가 본문(14px)보다 커진다(448px 시트에서 ~18px). 300px로 묶어 ~13px
- **판단 — 주석을 좌표까지 데이터로**: `<text>`에 인라인하면 금지어 스윕 밖으로 샌다(반복 교훈). 부수 효과로 **viewBox 밖 클리핑을 테스트로 잡는다** — 좌표 오타는 그림이 렌더되니 눈으로 놓친다
- ⚠️ **전제 정정 — 프로토타입 SVG를 이 세션에서 볼 수 없다**: 지시문은 "프로토타입 SVG를 그대로 이식"이었지만 `show_widget` 산출물은 이 대화에 없다. 이름(`candlesIcon`·`ILLUS.candle`…)·레이아웃·팔레트 지시에 맞춰 **18개를 직접 작도**했다. 원본과 픽셀 동일하지 않다
- ⚠️ **전제 정정 — 지시문 예시 주석이 직전 차수에서 걷어낸 표현이었다**: `"막대 0 위 = 상승 힘"`의 '상승 힘'은 **G1에서 라이브 `helpTexts`의 "상승 힘이 강하다는 신호"를 제거**한 바로 그 표현이다. 그림으로 되돌리면 회귀 → `"0 위 = 단기 평균이 장기 평균 위"`(사실 서술)로 쓰고, `상승 힘`·`하락 힘`·`너무 많이` 재유입을 테스트로 차단
- ⚠️ **전제 정정 — "현재는 색점+텍스트"는 칩이 아니라 범례**: 칩은 텍스트만이었다(색점은 오버레이 범례). 실제 상태는 지시문보다 나쁜 쪽 — 칩만 보고 지표를 구분할 단서가 **아무것도 없었다**
- ⚠️ **부수 수정(범위 밖) — 삽화가 팝업 세로를 늘려 작은 화면에서 '닫기'가 밀려났다**: 라이브 측정에서 360×740 전 9종 `닫기` 도달 실패. 시트를 `max-h-[88vh] flex-col`로 잡고 **본문만** 스크롤 + 닫기는 스크롤 밖 고정(MOB-4에서 DisclaimerModal에 쓴 처리). 부수적으로 옛 구조의 결함도 없어졌다 — `items-end` + 높이 무제한이라 내용이 길면 **제목이 위로 잘리고 스크롤도 안 됐다**. 360×560에서 시트 상·하단·제목·닫기·캐비엇 도달 전부 확인
- ⚠️ **하네스 결함 2건을 스스로 잡았다**(제품 결함 아님): ① `닫기` 도달 판정을 `버튼.bottom ≤ 시트.bottom`으로 했는데 **스크롤 컨테이너에서는 항상 실패**한다(스크롤 위치 밖 요소의 rect는 컨테이너를 넘는다) → 스크롤 후 재측정 + 고정 푸터로 구조 변경. ② 업스트림 `express-rate-limit`(120/분)에 걸려 페이지가 로딩 상태에 머물렀다 → **프록시에 GET 캐시**를 넣었다. 안 넣으면 스캔이 로딩 화면을 재는 **공회전**이 된다(2차 QA의 그 실패 형태)
- **육안 검토** — 삽화 9종 전부 스크린샷 확인. MACD가 세로로 눌려 두 선이 구분되지 않아 진폭을 2.5배로(막대 부호는 **두 선의 상하 관계와 정확히 일치**하게 — 주석이 "막대 = 두 선의 차이"라 어긋나면 그림이 거짓을 가르친다), 지시선 색 `#E7E7E3`→`#C9CAC6`(inset 배경에서 사실상 안 보였다)
- 검증: tsc 0 · next build ✓ · **npm test 718**(+48) · `FORBIDDEN_CHART` **∪** `FORBIDDEN_FLOW` 전수 스윕 0(삽화 주석은 캐비엇이 아니라 부정문 예외를 두지 않는다) · 문턱↔계산 잠금과 같은 방식으로 **칩 9종 ↔ 그림 키 집합 일치**를 테스트로 고정
- **라이브 검증**(로컬 dev + 캐싱 목 프록시, 프로덕션 쓰기 0): 칩 그리드 **12폭**(320~1440) 전부 아이콘 9/9 · 경계·글리프·문서 넘침 **0** · 팝업 삽화 9종 × PC/모바일 **18/18**(주석 viewBox 밖 0 · 캐비엇 존재 · 닫기 노출) · 토글 off→on→off 패널 생성·제거 · **콘솔 에러 0**
- **후속(범위 밖)**: 배당 렌즈·코포레이트 액션 분류 삽화는 신규 수집원이 필요해 Phase 2로 분리(마감 묶음과 동일 판정)
...
- ⚠️ **발견한 프로덕션 버그 2건(범위 밖 → 아래 부수 차수에서 처리)**: ① `investor_history.individual` 오염 ② `presets.ts:135`가 `foreign_sum`을 1e8로 나누며 `// 원 → 억`이라 주석했는데 실제 단위는 **순매매량(주)** — 거의 항상 0이 되어 라벨된 지표가 조용히 죽어 있다

**부수 — 투자자 데이터 표기 버그 정리 (2026-08-14)**

세션 3에서 드러난 표기 버그 2건. 비블로킹이라 수급 backfill과 병렬 처리. 원칙: **표시-계산 일치** + 측정 안 한 값을 만들지 않는다(R2).

- [x] **[BUG-1]** 스크리너 수급 배지 단위 오류 — `presetMetric`의 `foreign_buy`·`fund_buy`에서 `Math.round(sum / 1_0000_0000)`(`// 원 → 억`) 제거 → **항상 중립 문구**(`'외국인 순매수 중'`/`'기관 순매수 중'`). 두 프리셋은 정의상 순매수 상위라 매그니튜드 없이도 사실 전달에 손실이 없고, 초보자에게 순매매량(주)은 와닿지도 않는다. **금액(원) 환산 금지** — `price × shares`는 측정하지 않은 "순매수 금액"을 지어내는 것(R2). SSOT라 `/screener`·`/recommendations` 인라인 렌즈에 동시 전파
- **정정 — 지시문의 "집계가 데드가 된다"는 부정확**: `SUM(foreign_net)`은 `ORDER BY`·`HAVING`에 **계속 필요**하다. 데드가 되는 건 응답 **필드**(`foreign_sum`/`fund_sum`)의 표시 용도뿐 → 쿼리는 그대로 두고 라우터에 단위 주석만 남겼다
- [x] **[BUG-2]** `individual` 통일 — **라이브 경로는 `service.js`의 인라인 파서**이고, 거기서 `-(instNet + foreignNet)`으로 **역산**하고 있었다. 시장 참여자가 셋뿐이라 순매수 합이 0이라는 가정인데 기타법인·기타외국인이 있어 성립하지 않는다 → **측정하지 않은 값을 만들어 저장**하던 것. `null`로 교체(수급 채점은 `institution`·`foreign_net`만 사용, 읽는 곳 없음). 재스크래핑되는 날짜는 `ON CONFLICT DO UPDATE`로 자연히 덮인다 — 소급 정정 불요
- ⚠️ **정정 — 세션 3 보고의 "`naver.js:128`이 외국인 보유주수를 넣는다"는 대상이 틀렸다**: 그 `scrapeInvestorData`는 **호출처가 없는 죽은 코드**다(라이브 적재는 `service.js` 인라인 파서). 죽은 함수도 `individual: null`로 맞추고 미사용임을 주석에 명시
- 검증: node --check · tsc 0 · next build ✓ · **npm test 428**(+1 매그니튜드 스윕 — 0·음수·1억 주 초과에서 문구 고정·숫자 미노출) · 수급 점수 무영향

**부수 — 투자자 파서 3중화 통합 (2026-08-18)**

같은 네이버 `frgn` 표를 세 곳이 각자 파싱하던 것을 **파서 1개**로 접었다. 드리프트가 재발 가능한 상태 자체를 없애는 게 목적(직전 `individual` 불일치가 그 드리프트의 발현).

- [x] **[U-1]** 죽은 `naver.js` `scrapeInvestorData` **삭제**(호출처 0 재확인 후, 38줄). import 잔재 0
- [x] **[U-2]** `service.js` 라이브 적재를 `parseInvestorRows` 경유로 — 인라인 정규식 제거. **적재 결과 동일**이 목표라 옛 계약 3개를 그대로 지킨다: 20행 상한 · `foreign` 키(응답 계약 — `InvestorChart`·`interpretFlow`가 이 이름을 읽는다) · `.reverse()` 오름차순
- [x] **[U-3]** 동치 테스트 — **실물 HTML 픽스처**(`tests/fixtures/naver-frgn-005930.html`, 20행) + 삭제된 인라인 정규식을 **레거시 참조 구현으로 박제**. `date`·`institution`·`foreign` 전 행 일치. 비공회전 가드(픽스처가 실제로 15행 이상 + 부호가 섞여 있음)를 먼저 단언
...
**세션 3 결과 — 수급축 라이브 실행 (2026-08-14, 운영자 Neon)**

투자자 backfill 완료(129,270행 · 178/178종목 · 종목당 중앙값 727행 · 3년). `run.mjs` 재실행 — 수급 채점 23,071/24,386 시점. 산출물 `scripts/backtest/out/`: ic.csv·buckets.csv + `phase4_session3_summary.png`(300 DPI).

- **수급축도 무신호~약한 역방향**: train IC −0.014/−0.015/−0.015(5·20·60일), 전부 \|IC\|<0.02·**BH 후 생존 0**. holdout −0.011/−0.010/−0.004(음, 약). 버킷 **역방향**: 최강 순매수 구간 [1.5,2.0]이 최저(train 20일 1.30% < baseline 1.87% · test 20일 1.01% 최저). 비단조.
- **동시대성 경고는 안심 방향으로 해소**: 수급 IC가 **음(−)**이라, "동행이 예측처럼 보이는" 위양성(양의 IC 부풀림) 실패모드가 아니다. 오히려 플로우 추종이 역효과.
- **결정적 — 재구성된 7점(기술3+추세2+수급2) 전부 무신호~역방향, 양의 예측 신호 0.** 세션 1+2+3이 같은 결론. 남은 건 밸류 3점뿐인데, 전체 7/4 컷이 성립하려면 밸류가 **혼자서 7점의 역방향을 뒤집어야** 한다(5~60일에 밸류는 약한 팩터 — 실현성 낮음).
- **판정 — 밸류 역산 스파이크 DROP**: ROI 사실상 0. 측정된 70%가 역방향인 점수를 발행주식수 이력·과거 peer까지 풀어 3점 더 재봐야 결론 안 바뀜. **전략 질문 종료: 재구성 가능한 축에 예측력 없음 → 앱 가치는 전적으로 관찰 레이어(C·A·B·D)에 있다. Phase 4의 실질 산출 = "신호 품질 투자 중단" 실증 라이선스.**
- **프로덕션 함의**: MarketOpinion 내부 랭킹은 역방향 축 위에 서 있어 **높은 점수가 약하게 저성과 종목을 고른다**. 추천은 이미 에디터 큐레이션 전환·스크리너 프리셋은 중립 프레임이라 노출 제한적. `foreign_buy`/`fund_buy` 프리셋(순매수 상위)이 약하게 저성과 → **중립 "종목 탐색" 프레이밍이 실증적으로 정당**(좋은 매수로 프레이밍 금지). UI 변경 불필요, 단 "MarketOpinion=비예측(측정부는 역방향)" 팀 공유.

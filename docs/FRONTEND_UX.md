...
행동편향은 태생이 판단적이라 앱에서 **원칙 위반 위험이 가장 큰 영역** → 4.5c보다 더 좁게 강제한다. 순수 함수 `lib/journal/interpret.ts` + `tests/journal/interpret.test.ts` 전수 금지어 스윕(4.5c 금지어 + **질책·후회·손익판정어 + 물타기·편향** 확장)으로 회귀 차단.

- **전 영역 무채색** — 손익 수치도 방향색 금지(ink). 판정이 아닌 관찰이라 색으로 방향을 암시하지 않음 (3.13 규칙 최엄격 적용)
- **편향명은 flag=true일 때만** 부착 — 소표본·미세격차 과다명명 방지(처분효과 문턱 gap≥3일 & 이익/손실 각 n≥3). "물타기"·"편향" 같은 단정어는 금지 → "평단을 낮춘 매수" 같은 사실 서술로
- **provisional 뱃지** "실증 검증 전"(caution) — 임계값은 Phase 4 백테스팅 전 임시값임을 명시
- **인과 미서술** — 두 사실을 나란히만 두고 "때문에/그래서" 연결 금지 (R2: 이벤트 배지 O / 해석 라벨 X)
- **커버리지 캐비엇 상시 노출**(C-1) — 유니버스 제외(**건수≠종목수 구분**)·미매칭 매도 2축을 muted 각주로. 부분 히스토리면 open-lot 과대 가능성까지 고지
- **미실현 손실은 "최근 종가(asOfDate) 기준"**(C-2) — "지금" 금지(배치 데이터라 실시간 아님). "업로드하신 내역 기준" 스코프 명시
- **원칙**: 사실 + 중립 교육정의 + 관찰형("~하는 흐름이 보여요")까지만. 명령형(~하세요)·판단어(잘못/실수/손해/후회/좋다/나쁘/위험/사라/팔라) 전면 금지

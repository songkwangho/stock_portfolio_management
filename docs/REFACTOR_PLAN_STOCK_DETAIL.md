# 종목 상세 리팩터링 계획 (`app/stock/[code]/page.tsx`)

> 상태: **실행 완료 (2026-07-15, 커밋 `49493bc`~`642d4ac`)**. 아래는 원 계획 초안 + 실행 결과.
> 작성 근거: 커밋 `a955d1a` 기준 실제 측정값. Sprint 3 [M1] "차트 `components/charts/` 분리" 흡수 완료.
>
> **실행 결과 요약**
> - 셸 **1444 → 254줄**. `components/stock/detail/` 13개 + `lib/stockDetail/` 3개. 각 단계 tsc 0 error.
> - **계획 대비 변경**: (1) S5에서 PriceChart/VolumeChart 분리안 폐기 → `ChartSection` 통합. (2) 버그#3(StatsGrid↔chartTimeframe) → **S0 단독 커밋**으로 분리. (3) 크로스 마커 창 20일 확대 포함(SIG-4 정상화, 배포 30종목 중 22종목 마커 확인). (4) 헤더/사이드바 점수도 일봉 기준으로 결합 해소.
> - **목적 달성**: 구조 개선(컴포넌트 ≤181줄) ✅ / **리뷰 stall 복구 ❌** — 254줄 분해 후에도 동일 stall 재현. 원인 미확정(파일 크기 아님, 인프라 단정도 근거 부족).
> - **미수정 이슈**: 매수 폼 비중(%) 입력이 store `weight:0` 하드코딩으로 버려짐(3.12 이전부터, 범위 밖).
